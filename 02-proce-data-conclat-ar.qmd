...
data <- readRDS("output/data/clases-cl-ar.rds")
...
plot_binary_by <- function(
  ...
  fill_var = NULL,     # categórica para fill (opcional)
  ...
  palette = NULL,
  dodge_width = 0.8,
  single_fill = "#3b82f6",     # color único cuando no hay fill_var
  color_by_x = FALSE,          # si TRUE, colorea por categoría de x_var
  text_color = "white",        # color del texto de las etiquetas
  ...
  has_fill   <- !(quo_is_null(fill_quo) || quo_is_missing(fill_quo) || is.null(get_expr(fill_quo)))

  x_label    <- x_label    %||% as_label(enquo(x_var))
  if (has_fill) fill_label <- fill_label %||% as_label(fill_quo)
  ...
  if (has_fill) {
    df <- data %>%
      ...
      dplyr::group_by({{ x_var }}, {{ fill_var }}) %>%
      dplyr::summarise(prop = srvyr::survey_mean({{ var }}, na.rm = TRUE), .groups = "drop")
  ...
  y_scale <- if (isTRUE(expand_y)) {
    ggplot2::scale_y_continuous(labels = scales::percent_format(accuracy = axis_accuracy),
  ...
  .hjust <- if (is.null(x_text_hjust)) { if (x_text_angle == 0) 0.5 else 1 } else x_text_hjust
  .vjust <- if (is.null(x_text_vjust)) { if (x_text_angle == 90) 0.5 else 1 } else x_text_vjust
  ...
  if (isTRUE(save)) {
    if (is.null(file)) {
      if (!dir.exists("output/graph1")) dir.create("output/graph4", recursive = TRUE)
      if (has_fill) {
        file <- file.path("output/graph1", paste0(var_name, "_by_", x_name, "_and_", as_name(get_expr(fill_quo)), ".png"))
      } else {
        file <- file.path("output/graph1", paste0(var_name, "_by_", x_name, ".png"))
      }
    } else {
...
ggsave("output/graph1/edad.png", width = 10, height = 6, dpi = 300)
```
...
ggsave("output/graph1/edad_sexo.png", width = 10, height = 6, dpi = 300)
```
...
ggsave("output/graph1/class1.png", width = 10, height = 6, dpi = 300)
```
...
ggsave("output/graph1/class1_sexo.png", width = 10, height = 6, dpi = 300)
```
...
ggsave("output/graph1/class1_sexo_by_class.png", width = 10, height = 6, dpi = 300)
```
...
ggsave("output/graph1/class1_sexo_by_class_2.png", width = 10, height = 6, dpi = 300)
```
...
ggsave("output/graph1/class1_sexo_edad.png", plot = grafico, width = 10, height = 6, dpi = 300)
```
...
ggsave("output/graph1/politizacion_sexo_edad.png", width = 10, height = 6, dpi = 300)
```
...
ggsave("output/graph1/pol_id_sexo_edad.png", plot = grafico, width = 10, height = 6, dpi = 300)
```
...
# Calcular proporciones ponderadas solo por clase social
tabla_total <- data_svy %>%
  filter(!is.na(wrightclass2_reduced)) %>% 
...
ggsave("output/graph1/class2.png", width = 10, height = 6, dpi = 300)
```
...
# Calcular totales ponderados por clase x sexo
tabla_total <- data_svy %>%
  filter(!is.na(wrightclass2_reduced), !is.na(sexo)) %>%
  group_by(wrightclass2_reduced, sexo) %>%
  summarise(n = survey_total(vartype = NULL)) %>%
  ungroup() %>%
  # (opcional) asegurar combinaciones faltantes con 0
  complete(wrightclass2_reduced, sexo, fill = list(n = 0)) %>%
  group_by(sexo) %>%
  mutate(prop = n / sum(n)) %>%      # proporción dentro de cada sexo
  ungroup()
...
# Gráfico: barras agrupadas (dodge), normalizadas dentro de cada sexo
ggplot(tabla_total, aes(x = wrightclass2_reduced, y = prop, fill = sexo)) +
  geom_col(position = position_dodge(width = 0.8)) +
  scale_fill_manual(values = c("Mujer" = "#f25f5c", "Hombre" = "#9e1b1e")) +
  scale_y_continuous(labels = scales::percent_format(accuracy = 1)) +
  geom_text(
...
ggsave("output/graph1/class2_sexo.png", width = 10, height = 6, dpi = 300)
```
...
ggsave("output/graph1/class2_sexo_by_class.png", width = 10, height = 6, dpi = 300)
```
...
ggsave("output/graph1/class2_sexo_by_class_2.png", width = 10, height = 6, dpi = 300)
```
...
n_unw <- data_svy %>%
  filter(!is.na(wrightclass2_reduced), !is.na(sexo), !is.na(edad_tramos)) %>%
  summarise(N = unweighted(n())) %>%
  pull(N)
...
ggsave("output/graph1/class2_sexo_edad.png", plot = grafico, width = 10, height = 6, dpi = 300)
```
